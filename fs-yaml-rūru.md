...
fs:
  ...
  - value: 3
    ...
    mastertxt: GMだけに表示する演出メモ。PLには表示されない。
    check: 〈知覚〉
    difficulty: 8
...
- `recommended` は「PC4」のように suffix なしで表示されます。
- `recommended` には短い条件文も書けます。例: `PC4は事前調査資料あり。達成値+2`。
- 条件欄に表示する文言は25文字以内にします。画面側でも25文字で打ち切ります。
- どちらもない場合は空配列 `[]` にします。
- コロンの後には必ず空白を入れます。`recommended:PC4...` ではなく `recommended:` と改行して配列で書きます。
...
| `title` | 必須 | 進行値カードの見出し。 |
| `text` | 必須 | 進行値カードの本文。 |
| `mastertxt` | 任意 | その進行値での演出内容やGM用補足。GM画面だけに表示され、PL画面には表示されない。 |
